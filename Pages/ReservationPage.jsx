    ...
    const fetchData = async () => {
      try {
        ...
        emplacementsData.forEach((emplacement) => {
          typesCount[emplacement.type] =
            (typesCount[emplacement.type] || 0) + 1;
        });
        servicesData.forEach((service) => {
          servicesCount[service.libelle] =
            (servicesCount[service.libelle] || 0) + 1;
        });
    ...
    const filtrage = async () => {
      const filtré = await Promise.all(
        emplacements.map(async (emplacement) => {
          ...
          const resServicesEmplacements = await listeServicesEmplacements(
            emplacement.idEmplacement
          );
          const servicesEmplacements = resServicesEmplacements.data.map(
            (service) => service.libelle
          );
          console.log('servicesEmplacements', servicesEmplacements);

          const correspondanceTypes =
            filtres.types.length === 0 ||
            filtres.types.includes(emplacement.type);
          const correspondanceServices =
            filtres.services.length === 0 ||
            filtres.services.every((service) =>
              servicesEmplacements.includes(service)
            );
      ...
      <Container>
        ...
        <Row className="mt-5">
          <Col md={8}>
            <Row>
              {emplacementsFiltres.length > 0 ? (
                emplacementsFiltres.map((emplacement, index) => (
                ...
                <Card
                  ...
                  data-bs-theme="light"
                >
                  Il n'y a aucune correspondance disponible
                </Card>
              )}
            </Row>
          </Col>
          <Col md={4}>
            ...
            >
              <Card.Header as={'h4'} style={{ borderRadius: '5px' }}>
                Recherche avancée
              </Card.Header>
              <Form>
                <ListGroup>
                        ...
                        (service) => (
                          ...
                          (
                            ...
                            >
                              ...
                              />
                              <small className="text-muted">
                                ({counts.services[service.libelle]})
                              </small>
                            </div>
                          )
                        )
